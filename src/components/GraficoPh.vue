...
import type Ph from '@/interfaces/Ph'
...
const series = computed(() => [
  {
    data: ph.value.map((item) => parseFloat(item.value.toFixed(2)))
  }
])

onMounted(async () => {
  try {
    const response = await http.get('/ph')
...
<template>
  <div class="container">
    <div class="card text-center">
      <h3 class="text-info">Gráfico de PH</h3>
      <apexchart
